...
const downloadInteractions = async (page, selectedInteractionType) => {
    let filterPlantName = $("#filterPlantName").val()
    let filterInteractionType = selectedInteractionType
    let filterAnimalName = $("#filterAnimalName").val()
    let filterSource = $("#filterSource").val()
    ...
    return window.open(`/api/v1/interactions/${language}/download?page=${page}${filterPlantName}${filterInteractionType}${filterAnimalName}${filterSource}`, '_blank')
}