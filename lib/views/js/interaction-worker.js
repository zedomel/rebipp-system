...
const getDownloadMetadata = async (selectedInteractionType) => {
    ...
    const language = localStorage.getItem("language") || "en-US"
    const metadata = $.ajax({type: "GET", url: `/api/v1/interactions/${language}/download/metadata?page=0${filterPlantName}${filterInteractionType}${filterAnimalName}${filterSource}`, contentType: 'application/json' })
    return metadata
}

const downloadJsonInteractions = async (page, selectedInteractionType) => {
    let filterPlantName = $("#filterPlantName").val()
    let filterInteractionType = selectedInteractionType
    let filterAnimalName = $("#filterAnimalName").val()
    let filterSource = $("#filterSource").val()
    ...
    return window.open(`/api/v1/interactions/${language}/download/json?page=${page}${filterPlantName}${filterInteractionType}${filterAnimalName}${filterSource}`, '_blank')
}

const downloadCsvInteractions = async (page, selectedInteractionType) => {
    let filterPlantName = $("#filterPlantName").val()
    let filterInteractionType = selectedInteractionType
    let filterAnimalName = $("#filterAnimalName").val()
    let filterSource = $("#filterSource").val()
    ...
    return window.open(`/api/v1/interactions/${language}/download/csv?page=${page}${filterPlantName}${filterInteractionType}${filterAnimalName}${filterSource}`, '_blank')
}