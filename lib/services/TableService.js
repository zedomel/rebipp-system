...
export default class TableService {
    ...
    static async updateDQReportTablePublish(url, adminTableId, message) {
        ...
        rows.forEach(async (row, rowIndex) => {
            ...
            const dqReport = `datasets!I${rowIndex+1}`
            
            await Table.writeSpreadsheetRange(adminTableId, dqReport, message)
        })
    }
    ...
    static async dataAssessment(url) {
        const experimentId = url.split("/")[5]
        const readInteractionTable = async originalLanguage => {
            return Table.readSpreadsheetRange(url, `interaction.${originalLanguage}!A:GW`)
        }
        const createInteractionRecordsFromTable = (interactionTable, originalLanguage) => {
            ...
            return {schemaLayout, interactionTable}
        } 
        const sheetStrucutureValidator = tablesAvailable => {
            let isValid = true
            let message = null
            let tables = {}
            tablesAvailable.forEach(table => {
                tables[table.properties.title.toLowerCase()] = true
            })
            if(!tables["metadata"]) {
                isValid = false
            ...
            if(!tables["interaction.en-us"] && !tables["interaction.pt-br"]) {
                isValid = false
                message = 'Table "interaction.en-US" or "interaction.pt-BR" was not found'
                return {isValid, message}    
            }
            if(!tables["glossary.en-us"]) {
                isValid = false
                message = 'Table "glossary.en-US" was not found'
                return {isValid, message}    
            }
            if(!tables["metadata"]) {
                isValid = false
                message = 'Table "glossary-pt-BR" was not found'
                return {isValid, message}    
            }
            return {isValid, message}
        }
        const tablesAvailable = await Table.getTablesAvailable(url)
        const structureValidation = sheetStrucutureValidator(tablesAvailable)
        if(structureValidation.isValid === false) {
            return {structureValidation}
        }
        const originalLanguage = await Table.getOriginalLanguage(url)
        const interactionTable = await readInteractionTable(originalLanguage)
        const records  = createInteractionRecordsFromTable(interactionTable, originalLanguage)
        return {experimentId, originalLanguage, records}
    }
    static async addCommentsAndColorByColumn(url, columnIndex, notes, colors) {
        try {
            await new Promise(resolve => setTimeout(resolve, 1000))
            return await Table.addCommentAndColorToSpreadsheetColumn(url, columnIndex, notes, colors)
        } catch(error) {
            if (error.code === 429) {
                const waitTime = 2000;
                console.warn(`API rate limit exceeded. Retrying in ${waitTime} ms...`);
                await new Promise(resolve => setTimeout(resolve, waitTime))
                await TableService.addCommentsAndColorByColumn(url, columnIndex, notes, colors)
            }
        }
