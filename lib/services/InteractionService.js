...
export default class InteractionService {
    static async createRecordFromRow(row, schemaLayout, originalLanguage, experimentId, allTerms) {
        ...
        //TODO: allow addition of coluns in the data outside the standards (save them or discard?)
        ...
        const plantScientificName = "plant:dwc:Taxon:scientificName"
        const plantScientificNameIndex = schemaLayout.indexOf(plantScientificName)
        if (!plantScientificNameIndex == -1) {
            throw Error("Plant scientificName is required")
        }

        const animalScientificName = "animal:dwc:Taxon:scientificName"
        const animalScientificNameIndex = schemaLayout.indexOf(animalScientificName)
        if (!animalScientificNameIndex == -1) {
            throw Error("Animal scientificName is required")
        }

        const interactionType = "interaction:rebipp:Interaction:interactionType"
        const interactionTypeIndex = schemaLayout.indexOf(interactionType)
        if (!interactionTypeIndex == -1) {
            throw Error("interactionType is required")
        }
