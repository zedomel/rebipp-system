...
export default class InteractionService {
    static async createRecordFromRow(row, schemaLayout, originalLanguage, experimentId, allTerms) {
        ...
        terms.forEach(({schemaItem, termRecords}, index) => {                
            ...
            record[schemaItem]["originalValue"] = row && row[index] && row[index].trim && row[index].trim() || ''
            if(!termRecords) {
            ...
            if(termRecords.length > 1) { // states
                ...
                termRecords.forEach(termRecord => {
                    if(termRecord["state"] && termRecord["vocabulary"] && (termRecord["vocabulary"].toLowerCase() === record[schemaItem]["originalValue"].toLowerCase() || termRecord["state"].toLowerCase() === record[schemaItem]["originalValue"].toLowerCase())) {
